...
>
> 而面向过程语言大多都是直接编译成机械码在电脑上执行，但一些面向过程的脚本语言性能也并不一定比 Java 好。
>
> Android 的ART 虚拟机会使用AOT技术将APP在安装时直接编译为机器代码，之后运行的都是机器代码，速度很快。
...
hashCode（）介绍：hashCode() 的作用是获取哈希码，也称为散列码；它实际上是返回一个 int 整数。这个哈希码的作用是确定该对象在哈希表中的索引位置。hashCode() 定义在 JDK 的 Object.java 中，这就意味着 Java 中的任何类都包含有 hashCode() 函数。
...
![Java线程状态变迁](/images/Java 线程状态变迁.png)
...
![Java异常类层次结构图](/images/Exception.png)
...
![IO-操作方式分类](/images/IO-操作方式分类.png)

按操作对象分类结构图：

![IO-操作对象分类](/images/IO-操作对象分类.png)
